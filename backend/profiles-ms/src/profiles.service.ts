...
export class ProfilesService implements OnModuleInit {
  ...
  @logCall()
  async getUser(param: any): Promise<any> {
    return await lastValueFrom(
      this.toAuthProxy.send({ cmd: 'getUser' }, { ...param }),
    );
  }
  async getAllUsers(): Promise<any> {
    return await lastValueFrom(
      this.toAuthProxy.send({ cmd: 'getAllUsers' }, {}),
    );
  }
  @logCall()
  ...
  async getAllProfiles(): Promise<Profile[]> {
    ...
    const combined = mergeById(profiles, users);
    return combined;
  }
  @logCall()
